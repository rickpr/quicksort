...
// The partition function. This takes an array, and start and end points within
// the array. The end point is no longer always the pivot.
int partition (int *array, int initial, int end)
{
  // Put that random element at the end so the partition function doesn't have
  // to be completely rethought.
  swap(&array[random_pivot(initial, end)], &array[end]);
  // This now contains a random pivot element.
  int pivot = array[end];
  ...
  for (inspector = start; inspector < end; inspector++)
  {
    ...
    if (array[inspector] <= pivot)
    {
      ...
      // all the indexing outside the array and results in a cleaner return
      // statement.
      start++;
    }
  }
